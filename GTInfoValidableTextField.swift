...
class GTInfoValidableTextField: KLTextField, GTValidableViewInterface {
    ...
    var valueStorage: GTValidableValueStorage! {
        didSet {
            (rx.text <-> valueStorage.valueVar).addDisposableTo(disposeBag)
            setupEndEditingObservable()
        }
    }
...
//        self.addTarget(self, action:#selector(textChanged(sedner:)) , for: .editingChanged)
    ...
    var contentText: String? {
        print("inside: \(String(describing: self.valueStorage.valueVar.value))")
        return self.valueStorage.valueVar.value
    }
    ...
    func setupEndEditingObservable() {
        rx.controlEvent(UIControlEvents.editingDidEnd).debug("End Editing")
            .subscribe(onNext: {
                if self.valueStorage.valueVar.value == nil {
                    if case .picker(let value) = self.valueStorage.validateInfo.keyboardType! {
                        if let defaultSelection = value.first {
                            ...
                            self.valueStorage.updateValueString(with: defaultSelection.pickerTitle)
                            self.reload()
                        }else{
                        ...
                        if let defaultDate = self.datePicker?.date {
                            ...
                            self.valueStorage.updateValueString(with: dateString)
                            self.reload()
                        }
                    }
                }
    ...
    func clearView() {
        self.text = nil
        self.valueStorage.valueVar.value = nil
    ...
    func reload() {
        self.configure(with: self.valueStorage)
    }
    
    func configure(with valueStorage:GTValidableValueStorage){
        self.text = valueStorage.valueVar.value
...
//        self.valueStorage.validateInfo.validateDelegate = delegateTarget
    ...
    */
    ...
    func dateChanged(_ datePicker:UIDatePicker) {
        ...
        self.valueStorage.valueVar.value = self.text
    }
...
//    }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        self.valueStorage.valueVar.value = self.attributedText?.string
